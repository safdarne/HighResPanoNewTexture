...
/*
 ...
 Abstract:
 The VideoReader OpenGL effect renderer
 */
...
@interface VideoReaderOpenGLRenderer ( )
{
	EAGLContext *_oglContext;
	CVOpenGLESTextureCacheRef _textureCache;
	CVOpenGLESTextureCacheRef _renderTextureCache;
    CVOpenGLESTextureCacheRef _resizeTextureCache;
    ...
    CFDictionaryRef _resizeBufferPoolAuxAttributes;
    CMFormatDescriptionRef _resizeOutputFormatDescription;
    
    CVOpenGLESTextureRef _texture;
    CVPixelBufferRef _renderedPixelBuffer;
    
    float _cvResizeFactor;
...
@implementation VideoReaderOpenGLRenderer
...
- ( CVPixelBufferRef )copyRenderedPixelBuffer:( CVPixelBufferRef )pixelBuffer
{
	...
	static const float textureVertices[  ] = {
        // Upside down corrected at the intial stage instead of the last rendering
	...
	glFramebufferTexture2D(  GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, CVOpenGLESTextureGetTarget(  _texture  ), CVOpenGLESTextureGetName(  _texture  ), 0  );
	...
	glBindTexture(  CVOpenGLESTextureGetTarget(  _texture  ), 0  );
...
bail:
    ...
    _renderedPixelBuffer = dstPixelBuffer;
...
- ( CVPixelBufferRef )copyRenderedPixelBufferWithResize:( CVPixelBufferRef )pixelBuffer
{
    ...
    float textureVertices2[  ] = {
         // Upside down once more to make it correct for OpenCV image
    ...
    for ( int i = 0; i < 8; i++ )
        textureVertices2[ i ] /=_cvResizeFactor;
    ...
    const CMVideoDimensions srcDimensions = { ( int32_t )CVPixelBufferGetWidth( pixelBuffer ), ( int32_t )CVPixelBufferGetHeight( pixelBuffer ) };
    const CMVideoDimensions dstDimensions = CMVideoFormatDescriptionGetDimensions(  _resizeOutputFormatDescription  );
    /*
    if (  srcDimensions.width != dstDimensions.width || srcDimensions.height != dstDimensions.height  ) {
        @throw [ NSException exceptionWithName:NSInvalidArgumentException reason:@"Invalid pixel buffer dimensions" userInfo:nil ];
        return NULL;
    ...
    */
    ...
    err = CVPixelBufferPoolCreatePixelBufferWithAuxAttributes(  kCFAllocatorDefault, _resizeBufferPool, _resizeBufferPoolAuxAttributes, &dstPixelBuffer  );
    if (  err == kCVReturnWouldExceedAllocationThreshold  ) {
        // Flush the texture cache to potentially release the retained buffers and try again to create a pixel buffer
        CVOpenGLESTextureCacheFlush(  _renderTextureCache, 0  );
        err = CVPixelBufferPoolCreatePixelBufferWithAuxAttributes(  kCFAllocatorDefault, _resizeBufferPool, _resizeBufferPoolAuxAttributes, &dstPixelBuffer  );
    ...
    err = CVOpenGLESTextureCacheCreateTextureFromImage(  kCFAllocatorDefault,
                                                       ...
                                                       GL_TEXTURE_2D,
                                                       GL_RGBA,
                                                       dstDimensions.width , // changing this causes partial read, should match size of dstPixelBuffer
                                                       dstDimensions.height ,
                                                       GL_BGRA,
                                                       GL_UNSIGNED_BYTE,
                                                       0,
    ...
    glEnableVertexAttribArray(  ATTRIB_VERTEX  );
    glVertexAttribPointer(  ATTRIB_TEXTUREPOSITON, 2, GL_FLOAT, 0, 0, textureVertices2  );
    glEnableVertexAttribArray(  ATTRIB_TEXTUREPOSITON  );
...
- ( BOOL )initializeBuffersWithOutputDimensions:( CMVideoDimensions )outputDimensions retainedBufferCountHint:( size_t )clientRetainedBufferCountHint
{
    ...
    err = CVOpenGLESTextureCacheCreate(  kCFAllocatorDefault, NULL, _oglContext, NULL, &_resizeTextureCache  );
    if (  err  ) {
        NSLog(  @"Error at CVOpenGLESTextureCacheCreate %d", err  );
        success = NO;
        goto bail;
	...
	const GLchar *vertSrc = [ VideoReaderOpenGLRenderer readFile:@"myFilter.vsh" ];
	const GLchar *fragSrc = [ VideoReaderOpenGLRenderer readFile:@"myFilter.fsh" ];
    ...
    //// Mori
    //size_t maxRetainedBufferCount = clientRetainedBufferCountHint;
    _resizeBufferPool = createPixelBufferPool(  outputDimensions.width * _cvResizeFactor, outputDimensions.height * _cvResizeFactor, kCVPixelFormatType_32BGRA, ( int32_t )maxRetainedBufferCount  ); //// fixme, set size here
    if (  ! _resizeBufferPool  ) {
        NSLog(  @"Problem initializing a buffer pool."  );
        success = NO;
        goto bail;
    }
    
    _resizeBufferPoolAuxAttributes = createPixelBufferPoolAuxAttributes(  ( int32_t )maxRetainedBufferCount  );
    preallocatePixelBuffersInPool(  _resizeBufferPool, _resizeBufferPoolAuxAttributes  );
    
    //CMFormatDescriptionRef outputFormatDescription = NULL;
    //CVPixelBufferRef testPixelBuffer = NULL;
    CVPixelBufferPoolCreatePixelBufferWithAuxAttributes(  kCFAllocatorDefault, _resizeBufferPool, _resizeBufferPoolAuxAttributes, &testPixelBuffer  );
    if (  ! testPixelBuffer  ) {
        NSLog(  @"Problem creating a pixel buffer."  );
        success = NO;
        goto bail;
    ...
    CMVideoFormatDescriptionCreateForImageBuffer(  kCFAllocatorDefault, testPixelBuffer, &outputFormatDescription  );
    _resizeOutputFormatDescription = outputFormatDescription;
    CFRelease(  testPixelBuffer  );
    ////
...
- ( void )deleteBuffers
{
    ...
    if (  _resizeTextureCache  ) {
        CFRelease(  _resizeTextureCache  );
        _resizeTextureCache = 0;
    }
	if (  _bufferPool  ) {
